...
internal class EmotionGridList(
    ...
    private fun renderFade(
        ...
        repeat(EmotionPickerEdgeFade.HEIGHT) { offset ->
            ...
            val color = alpha shl 24 or (EmotionPickerEdgeFade.color and 0x00FFFFFF)
...
private class EmotionIconButton(
    ...
    private var cachedSourceWidth = -1
    private var cachedLabelSource = ""
    ...
    override fun renderWidget(guiGraphics: GuiGraphics, mouseX: Int, mouseY: Int, partialTick: Float) {
        ...
        val label = fittedLabel(font, message.string, availableLabelWidth)
    ...
    private fun fittedLabel(font: Font, value: String, availableWidth: Int): String {
        val sourceWidth = font.width(value)
        if (
            cachedLabelWidth == availableWidth &&
            cachedSourceWidth == sourceWidth &&
            cachedLabelSource == value
        ) {
            return cachedLabel
        }
        cachedLabelWidth = availableWidth
        ...
        cachedLabel = if (sourceWidth <= availableWidth) {
            ...
            EmotionLabelTruncation.completePrefix(value, shortened) + TRUNCATION_MARK
        }
        return cachedLabel
    }
...
internal class EmotionTabButton(
    ...
    private fun renderLabel(guiGraphics: GuiGraphics) {
        ...
        if (
            ...
            tooltip = if (cachedLabel == source) null else fullTooltip
        }
        guiGraphics.drawString(
            font,
            cachedLabel,
            x + (width - font.width(cachedLabel)) / 2,
