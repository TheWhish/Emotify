...
class ServerPlayerSession(
    ...
    private val rejectionResponses = TokenBucket(
        capacity = REJECTION_BURST_CAPACITY,
        refillTokensPerSecond = REJECTION_REFILL_TOKENS_PER_SECOND,
    ...
    fun receiveClientHello(hello: ClientHello): ServerHandshakeTransition {
        return when (val current = handshakeState) {
    ...
    private fun establish(hello: ClientHello): ServerHandshakeTransition {
        return when (val negotiated = ProtocolNegotiator.negotiate(serverCapabilities, hello.capabilities, featureRegistry)) {
    ...
    companion object {
        private const val REJECTION_BURST_CAPACITY = 2
        private const val REJECTION_REFILL_TOKENS_PER_SECOND = 1
