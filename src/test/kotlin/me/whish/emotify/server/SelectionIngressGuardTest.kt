...
class SelectionIngressGuardTest : FunSpec({
    ...
    test("ten thousand known selections forward only the initial burst") {
        ...
        repeat(10_000) {
            if (guard.shouldForward(emotionId, EmotionCatalog.BUILT_IN)) {
                forwarded++
            }
        }
    ...
    test("unknown selections never reach gameplay validation") {
        val guard = SelectionIngressGuard(FakeMonotonicTimeSource())
        val emotionId = EmotionId.of("external:unknown")
        var forwarded = 0
    ...
    test("only one selection can wait for the server main thread") {
        ...
        guard.tryReserveMainThreadTask(emotionId, EmotionCatalog.BUILT_IN) shouldBe true
        guard.tryReserveMainThreadTask(emotionId, EmotionCatalog.BUILT_IN) shouldBe false
