...
class EmotionBillboardLayoutTest : FunSpec({
    test("upright icon stays close to the standing head") {
        EmotionBillboardLayout.ICON_SIZE.shouldBeExactly(0.32f)
        ...
        EmotionBillboardLayout.localY(1.8, 0.0, Pose.STANDING) shouldBe (2.2 plusOrMinus 1.0e-12)
        EmotionBillboardLayout.bottomClearance(Pose.STANDING) shouldBe (0.24 plusOrMinus 1.0e-12)
    }
    ...
    test("prone poses use their compact visual dimensions without interpolation") {
        listOf(Pose.FALL_FLYING, Pose.SWIMMING, Pose.SPIN_ATTACK).forEach { pose ->
            ...
            EmotionBillboardLayout.localY(0.6, 0.0, pose) shouldBe (1.2 plusOrMinus 1.0e-12)
            EmotionBillboardLayout.bottomClearance(pose) shouldBe (0.44 plusOrMinus 1.0e-12)
        }
    }
    ...
    test("sleeping pose uses a dedicated bed-safe anchor") {
        ...
        EmotionBillboardLayout.localY(0.2, 0.0, Pose.SLEEPING).shouldBeExactly(1.0)
        EmotionBillboardLayout.bottomClearance(Pose.SLEEPING) shouldBe (0.64 plusOrMinus 1.0e-12)
    }
    ...
    test("fall flying anchor follows the vanilla rotation transition") {
        EmotionBillboardLayout.fallFlyingLocalY(2.2, 1.2, 0, 0.0f) shouldBe
            (2.2 plusOrMinus 1.0e-12)
        EmotionBillboardLayout.fallFlyingLocalY(2.2, 1.2, 5, 0.0f) shouldBe
            (1.95 plusOrMinus 1.0e-12)
        EmotionBillboardLayout.fallFlyingLocalY(2.2, 1.2, 10, 0.0f) shouldBe
            (1.2 plusOrMinus 1.0e-12)
    }

    test("fall flying transition clamps malformed frame inputs") {
        EmotionBillboardLayout.fallFlyingLocalY(2.2, 1.2, -5, -1.0f) shouldBe
            (2.2 plusOrMinus 1.0e-12)
        EmotionBillboardLayout.fallFlyingLocalY(2.2, 1.2, 20, 2.0f) shouldBe
            (1.2 plusOrMinus 1.0e-12)
    }
})
