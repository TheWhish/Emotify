...
class ClientHandshakeSessionTest : FunSpec({
    ...
    val validHello = ServerHello(localCapabilities, 1_200, EmotionCatalog.BUILT_IN)

    test("pending connection times out at exactly five seconds") {
        val time = FakeMonotonicTimeSource()
        val session = ClientHandshakeSession(localCapabilities, EmotionCatalog.BUILT_IN, time)
    ...
    test("major mismatch disables only the feature and still reports client capabilities") {
        val session = ClientHandshakeSession(localCapabilities, EmotionCatalog.BUILT_IN, FakeMonotonicTimeSource())
