...
class ClientHandshakeSessionTest : FunSpec({
    ...
    test("connection generations must be positive") {
        val session = ClientHandshakeSession(localCapabilities, BuiltInEmotionCatalog.catalog, FakeMonotonicTimeSource())

        shouldThrow<IllegalArgumentException> { session.begin(connectionId = 0L) }
        shouldThrow<IllegalArgumentException> { session.begin(connectionId = -1L) }
