...
object CustomEmojiRegistry {
    ...
    fun resolveTexture(textureId: String): Identifier? = snapshot.textureById[textureId]
    ...
    private fun load(
        ...
        try {
            ...
            val texture = Identifier.fromNamespaceAndPath(CUSTOM_NAMESPACE, asset.id.hexValue())
    ...
    private fun DecodedCustomEmoji.toAtlas(): NativeImage {
        ...
        try {
            frames.forEachIndexed { frameIndex, frame ->
                val framePixels = frame.pixelsABGR
    ...
    private fun NativeImage.toCustomEmojiPixels(): CustomEmojiPixels = CustomEmojiPixels.of(width, pixelsABGR)
    ...
    private fun apply(minecraft: Minecraft, loaded: LoadedCustomEmojiLibrary) {
        ...
        val registeredTextures = ArrayList<Identifier>(loaded.entries.size)
        val publication = try {
            ...
            retainedEntries.values.forEach { entry ->
                ...
                val texture = try {
                    DynamicTexture({ "Emotify local custom emoji" }, image)
                } catch (failure: Throwable) {
    ...
    private fun rollbackPublication(
        ...
        registeredTextures: List<Identifier>,
        failure: Throwable,
    ): Nothing {
...
private data class CustomEmojiSnapshot(
    ...
    val textureById: Map<String, Identifier>,
