...
object EmotionPickerMovement {
    ...
    private fun updateKeyboardState(
        ...
        var matched = false
        matched = trackKeyboardBinding(options.keyUp, PickerMovementAction.FORWARD, keyCode, scanCode, down) or matched
        matched = trackKeyboardBinding(options.keyDown, PickerMovementAction.BACKWARD, keyCode, scanCode, down) or matched
        matched = trackKeyboardBinding(options.keyLeft, PickerMovementAction.LEFT, keyCode, scanCode, down) or matched
        matched = trackKeyboardBinding(options.keyRight, PickerMovementAction.RIGHT, keyCode, scanCode, down) or matched
        matched = trackKeyboardBinding(options.keyJump, PickerMovementAction.JUMP, keyCode, scanCode, down) or matched
        matched = trackKeyboardBinding(options.keyShift, PickerMovementAction.CROUCH, keyCode, scanCode, down) or matched
        matched = trackKeyboardBinding(options.keySprint, PickerMovementAction.SPRINT, keyCode, scanCode, down) or matched
        return matched
    }
    ...
    private fun trackKeyboardBinding(
        ...
        if (KeyBindingHelper.getBoundKeyOf(binding).type == InputConstants.Type.SCANCODE) {
    ...
    private fun isPhysicallyDown(binding: KeyMapping, action: PickerMovementAction, window: Long): Boolean {
        val key = KeyBindingHelper.getBoundKeyOf(binding)
