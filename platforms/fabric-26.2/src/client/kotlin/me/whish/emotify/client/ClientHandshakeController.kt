...
object ClientHandshakeController {
    ...
    fun register() {
        ...
        ClientPlayConnectionEvents.DISCONNECT.register { listener, minecraft ->
            minecraft.execute { disconnect(listener) }
        }
        ServerboundPlayChannelEvents.REGISTER.register { listener, _, _, channels ->
            if (activeListener === listener && channels.any(clientProtocolChannels::contains)) {
                retryDeferredServerHello(listener)
            }
        }
        ServerboundPlayChannelEvents.UNREGISTER.register { listener, _, _, channels ->
            if (activeListener === listener && channels.any(clientProtocolChannels::contains)) {
                markProtocolChannelsUnavailable()
            }
        }
        ClientLevelEvents.AFTER_CLIENT_LEVEL_CHANGE.register { _, _ -> clearWorldState() }
    ...
    private fun receive(listener: ClientPacketListener, chunk: CustomEmojiAssetChunk) {
        ...
        when (customAssetDecoder.submit(activeConnectionId, chunk)) {
            ...
            RemoteCustomAssetAdmission.CLOSED -> EmotifyFabric.LOGGER.error(
