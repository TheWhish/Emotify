...
object FabricServerLifecycle {
    ...
    fun register() {
        ...
        ClientboundConfigurationChannelEvents.REGISTER.register { handler, _, server, channels ->
            if (FabricChannelSupport.registerConfigurationChannels(handler, channels)) {
                reconcileConfigurationSupport(server, handler)
            }
        }
        ClientboundConfigurationChannelEvents.UNREGISTER.register { handler, _, server, channels ->
        ...
        ClientboundPlayChannelEvents.REGISTER.register { handler, _, server, channels ->
            if (FabricChannelSupport.registerPlayChannels(handler.player, channels)) {
                openOrRefreshIfSupported(server, handler.player)
            }
        }
        ClientboundPlayChannelEvents.UNREGISTER.register { handler, _, server, channels ->
