...
@Suppress("unused")
class ProtocolV1NeoForgeParityTest {
    val capabilities = ProtocolCapabilities(ProtocolVersion.CURRENT, FeatureFlags.NONE)

    @Test
    fun `NeoForge adapters match every frozen valid wire fixture`() {
    ...
    @Test
    fun `NeoForge adapters match pure rejection for every malicious fixture`() {
    ...
    @Test
    fun `NeoForge adapter matches duplicate catalog semantic results`() {
        listOf(
    ...
    @Test
    fun `NeoForge prepared custom asset body is reused across fanout`() {
