...
@Suppress("unused")
class EmotifyServerConfigSpecTest {
    @Test
    fun `empty NeoForge server configuration receives the complete audited defaults`() {
    ...
    @Test
    fun `future NeoForge server schema remains opaque to automatic correction`() {
        val config = CommentedConfig.inMemory()
    ...
    @Test
    fun `explicit legacy NeoForge schema is corrected once while valid values survive`() {
        val config = CommentedConfig.inMemory()
