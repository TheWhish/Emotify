...
class HelloPayloadCodecTest {
    ...
    @Test
    fun `server hello round trips through its wire codec`() {
        roundTrip(ServerHelloPayload.STREAM_CODEC, serverPayload) shouldBe serverPayload
    }

    @Test
    fun `complete built in catalog remains compact during handshake`() {
    ...
    @Test
    fun `client hello round trips through its wire codec`() {
        roundTrip(ClientHelloPayload.STREAM_CODEC, clientPayload) shouldBe clientPayload
    }

    @Test
    fun `server hello decoder accepts a three hundred entry legacy catalog`() {
    ...
    @Test
    fun `server hello decoder accepts its exact legacy maximum while outbound profile rejects it`() {
        val catalog = EmotionCatalog.of(
    ...
    @Test
    fun `server hello outbound profile accepts four thousand ninety six and rolls back four thousand ninety seven`() {
    ...
    @Test
    fun `server hello rejects more than five hundred and twelve catalog entries`() {
    ...
    @Test
    fun `server hello preserves duplicate emotion IDs as a semantic failure`() {
        val buffer = FriendlyByteBuf(Unpooled.buffer())
        try {
            buffer.writeByte(1)
            buffer.writeByte(0)
    ...
    @Test
    fun `hello codecs reject trailing bytes`() {
    ...
    @Test
    fun `hello codecs reject non canonical feature flags`() {
        val buffer = FriendlyByteBuf(Unpooled.buffer())
        try {
            buffer.writeByte(1)
            buffer.writeByte(0)
    ...
    @Test
    fun `client hello accepts a canonical ten byte feature VarLong`() {
    ...
    @Test
    fun `client hello rejects an eleven byte feature VarLong`() {
        val buffer = FriendlyByteBuf(Unpooled.buffer())
        try {
            buffer.writeByte(1)
            buffer.writeByte(0)
    ...
    @Test
    fun `client hello accepts its twelve byte maximum and rejects a thirteenth byte`() {
