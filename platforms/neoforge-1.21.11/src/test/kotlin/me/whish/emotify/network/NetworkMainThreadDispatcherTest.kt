...
@Suppress("unused")
class NetworkMainThreadDispatcherTest {
    @Test
    fun `asynchronous success runs task and completion once`() {
    ...
    @Test
    fun `synchronous completed future runs completion once`() {
        val taskCalls = AtomicInteger()
        val completionCalls = AtomicInteger()
    ...
    @Test
    fun `runtime task failure is handled without exceptional completion`() {
        val future = CompletableFuture<Void>()
    ...
    @Test
    fun `synchronous enqueue failure is handled and completes cleanup`() {
    ...
    @Test
    fun `future cancellation before task execution completes cleanup`() {
        val future = CompletableFuture<Void>()
        val taskCalls = AtomicInteger()
    ...
    @Test
    fun `exceptional future before task execution reports enqueue failure and completes cleanup once`() {
        val future = CompletableFuture<Void>()
    ...
    @Test
    fun `an exceptional future after task start does not duplicate task diagnostics`() {
        val future = CompletableFuture<Void>()
        val expected = IllegalStateException("task failed")
    ...
    @Test
    fun `cleanup runs once when asynchronous enqueue diagnostics fail`() {
        val expected = IllegalStateException("asynchronous enqueue failed")
        val failureCalls = AtomicInteger()
        val completionCalls = AtomicInteger()
    ...
    @Test
    fun `guarded asynchronous fatal failure completes cleanup`() {
    ...
    @Test
    fun `synchronous fatal failure completes cleanup and propagates`() {
