...
class CustomEmojiImageDecoderTest {
    @Test
    fun `PNG decoding preserves the final row of a sixteen pixel image`() {
        ...
        try {
            ...
            image.pixelABGR(0, 14) shouldBe 0xFF302010.toInt()
            image.pixelABGR(0, 15) shouldBe 0xFF3366CC.toInt()
        } finally {
            decoded.close()
        }
    }
    ...
    @Test
    fun `JPEG decoding creates an opaque native image with the expected channel order`() {
    ...
    @Test
    fun `GIF decoding preserves composed frames and bounds their effective frame rate`() {
        ...
        try {
            ...
            decoded.frames[0].pixelABGR(0, 0) shouldBe 0xFF0000FF.toInt()
            decoded.frames[1].pixelABGR(0, 0) shouldBe 0xFF00FF00.toInt()
        } finally {
            decoded.close()
        }
    }
    ...
    @Test
    fun `GIF over the network frame budget is normalized without losing its tail`() {
        ...
        try {
            ...
            decoded.frames.first().pixelABGR(0, 0) shouldBe 0xFF0000FF.toInt()
            decoded.frames.last().pixelABGR(0, 0) shouldBe 0xFF00FF00.toInt()
        } finally {
            decoded.close()
        }
    }
    ...
    @Test
    fun `GIF structure is rejected before native decode when it exceeds the source safety budget`() {
    ...
    @Test
    fun `long GIF timing is clipped at the emotion lifecycle without global acceleration`() {
