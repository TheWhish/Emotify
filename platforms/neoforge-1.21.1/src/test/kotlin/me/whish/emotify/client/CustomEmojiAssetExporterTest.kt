            ...
            val sourceFrames = source.frames.mapIndexed { index, image ->
                CustomEmojiFrame(
                    CustomEmojiPixels.of(IntArray(image.width * image.height) { pixel ->
                        image.getPixelRGBA(pixel % image.width, pixel / image.width)
                    }),
            ...
            try {
                ...
                repeat(source.frameCount) { frameIndex ->
                    ...
                    repeat(16 * 16) { pixelIndex ->
                        exported.frames[frameIndex].getPixelRGBA(pixelIndex % 16, pixelIndex / 16) shouldBe
                            source.frames[frameIndex].getPixelRGBA(pixelIndex % 16, pixelIndex / 16)
                    }
                }
            } finally {
                exported.close()
    ...
    test("animated export clears an opaque frame before a transparent frame") {
        ...
        try {
            ...
            try {
                ...
                exported.frames[1].getPixelRGBA(0, 0) shouldBe transparent[0]
                exported.frames[1].getPixelRGBA(1, 0) shouldBe transparent[1]
            } finally {
                exported.close()
            }
        } finally {
