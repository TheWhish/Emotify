...
@Suppress("unused")
class ServerHelloRetryQueueTest : FunSpec({
    test("drain allows a bounded retry to be scheduled for the next tick") {
    ...
    test("stale disconnect cannot remove a replacement connection retry") {
        val queue = ServerHelloRetryQueue()
        val playerId = UUID.randomUUID()
        queue.schedule(playerId, connectionId = 1L, attemptsRemaining = 2)
