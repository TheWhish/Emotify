...
class RemoteCustomEmojiLookupTest : FunSpec({
    ...
    test("remote registry rejects invalid and stale connection lifecycles before decoding") {
        val asset = CustomEmojiAsset.create(CustomEmojiPixels.of(IntArray(64) { it }))

        shouldThrow<IllegalArgumentException> {
            RemoteCustomEmojiRegistry.begin(0L)
        }
        RemoteCustomEmojiRegistry.begin(41L)
        RemoteCustomEmojiRegistry.register(42L, asset) shouldBe false
        RemoteCustomEmojiRegistry.disconnect(41L)
        RemoteCustomEmojiRegistry.register(41L, asset) shouldBe false
    }
})
